		...
		if(!do_frontend())
			...
			for(i = 1; i < margc;i++)
				...
				if (strcasecmp(margv[i],"-halfscale") == 0)
					video_scale=2;
				if (strcasecmp(margv[i],"-bestscale") == 0)
					video_scale=3;
				if (strcasecmp(margv[i],"-fastscale") == 0)
					video_scale=4;
				...
				if (strcasecmp(margv[i],"-fullscale") == 0)
					video_scale=8;
				if (strcasecmp(margv[i],"-scandouble") == 0)
					video_scale=9;
				if (strcasecmp(margv[i],"-border") == 0)
					video_border=1;
				if (strcasecmp(margv[i],"-aspect") == 0)
					video_aspect=1;
				if (strcasecmp(margv[i],"-nothrottle") == 0)
					throttle=0;
			...
			res = frontend_help(margc, margv);
				...
				parse_cmdline (margc, margv, game_index);
			...
			{	/* Mish:  I need sample rate initialised _before_ rom loading for optional rom regions */
				...
				for (i = 1; i < argc; i++)
				{
					if (strcasecmp(margv[i],"-record") == 0)
					{
				    i++;
						if (i < argc)
							options.record = osd_fopen(margv[i],0,OSD_FILETYPE_INPUTLOG,1);
					}
				}
